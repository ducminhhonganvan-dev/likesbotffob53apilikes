...
api_keys = {
    "minhdevzz": {"exp": "30/07/2095", "remain": 999, "max_remain": 999, "last_reset": None}
}
...
async def get_token(acc, session):
    try:
        ...
        url = f"http://180.93.52.133:2035/token?key=tungdzvcl&uid={uid}&password={password}&region=SG"
...
async def refresh_token():
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as s:
            gringay = await get_token("4792785495:nhathoaz_J7O3B_BY_STAR_GMR_YA6Z8", s)
    except Exception:
        pass
